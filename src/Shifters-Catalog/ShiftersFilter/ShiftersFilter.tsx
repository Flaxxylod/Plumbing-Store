...
//НАПИСАТЬ UNIT-ТЕСТЫ к компоненту
const ShiftersFilter = () => {
    ...
    return (
        <div className={`lg:block ${MobileFilter_Open ? "block" : "hidden"} lg:border-[var(--orange-color)] lg:border-[2px]   max-lg:fixed max-lg:z-30 max-lg:top-0 max-lg:left-0 max-lg:right-0 max-h-[679px]`}>
            ...
            <div className="relative  md:overflow-y-auto md:overscroll-y-contain flex flex-col gap-y-[24px] lg:p-[28px] p-[16px]  bg-[var(--white-color)] lg:border-[var(--orange-color)]  lg:w-[282px] max-lg:h-screen max-lg:w-screen">
                ...
                <div className="lg:block flex flex-col justify-between ">
                    ...
                    <div className="shiferfilterblock mb-[16px] ">
                        ...
                        <ul className="flex flex-col gap-y-[12px] mt-[16px] max-md:overflow-y-scroll max-h-[176px]">
                            {CheckboxList.map((item) => (
                                <CheckboxFilter Key={item.id} isChanged={() => selectCheckbox(item.id)} isChecked={Selected_Checkbox.includes(item.id)}>{item.label}</CheckboxFilter >
                            )).slice(2)}
                        </ul>
