...
const ProductCard_Info = ({ product }) => {
    if (!product) return <div>загрузка данных</div>
    return (
        <div className="flex flex-col max-w-[482px] p-[28px_32px] border-[var(--orange-color)] border-[2px]">
            <h3 className="tracking-normal!">
                <span className="w-full text-[#424242] font-[400] text-[20px]">{product.title}</span>
            </h3>

            <div className="relative flex flex-col">
                <strong>
                    {product?.price || "загрузка.."} ₽
                </strong>
